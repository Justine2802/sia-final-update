  ...
  useEffect(() => {
    if (resident) {
      setFormData({
        ...
        is_voter: String(resident.is_voter) || '0',
        is_verified: String(resident.is_verified) || '0'
      });
    } else {
      setFormData({ 
    ...
    try {
      if (resident?.id) {
        ...
        await residentsAPI.create({
            ...formData,
            // Assuming your backend needs a default password for new resident accounts
            password: 'password123', 
            password_confirmation: 'password123'
        });
      }
      onClose(); // Triggers the refresh in the parent component
    } catch (err) {
      console.error(err);
      setError(err.response?.data?.message || 'Failed to save resident record. Check database constraints.');
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="p-10 space-y-8 text-left max-h-[70vh] overflow-y-auto custom-scrollbar">
            ...
            <div>
                ...
                <div>
                    ...
                    <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Optional for portal access"
                        className="w-full pl-14 pr-6 py-4 bg-gray-50 border border-gray-100 rounded-[20px] font-bold outline-none" />
                    </div>
                </div>
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <select name="purok" value={formData.purok} onChange={handleChange}
                        className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-[20px] font-bold text-gray-700 outline-none appearance-none">
